...
(defn render-document
  ...
  (let [document (if (instance? SVGDocument doc-file)
                   doc-file
...
(defn get-pipelined-gif
  ...
  (let [svg-doc (open-svg-doc svg-file)
        ...
        gif-stream  (let [href (.getAttribute image-node "href")
                      ...
                      (b64/base64-string->byte-array (when base64-pos
                                                       (subs href (+ base64-pos (count href-base64))))))

        caption-pixel-height (math/ceil (* (/ (- virtual-height content-virtual-height) content-virtual-height) content-pixel-height))
